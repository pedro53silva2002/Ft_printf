...
/*   Updated: 2024/05/06 16:50:31 by peferrei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_check(unsigned char str, va_list list_va)
{
	...
	nbr = 0;
	if (str == 's')
		nbr += ft_putstr_fd(va_arg(list_va, char *), 1);
	else if (str == 'd' || str == 'i')
		nbr += ft_putnbr_fd(va_arg(list_va, int), 1);
	else if (str == 'c')
		nbr += ft_putchar_fd(va_arg(list_va, int), 1);
	else if (str == 'x')
		nbr += ft_int_puthex(va_arg(list_va, unsigned long), 'x');
	else if (str == 'X')
		nbr += ft_int_puthex(va_arg(list_va, unsigned long), 'X');
	else if (str == 'u')
		nbr += ft_putnbr_base(va_arg(list_va, unsigned int), "ude");
	else if (str == 'p')
		nbr += ft_putptr(va_arg(list_va, unsigned long));
	else if (str == '%')
	{
		write(1, "%", 1);
		nbr += 1;
